...
{
    public function custom_index(Training $training, Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            $query = $model::select($dataType->name.'.*')->whereBelongsTo($training);
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                $redirect = redirect()->route("sessions", $data->training_id);
            } else {
